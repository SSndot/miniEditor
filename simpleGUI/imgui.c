...
#define WordWidth 0.03

#define LeftWidth GetFontAscent()/2 + 0.2 - 0.02
double RowPitch = 0.2;
extern int cflag;
extern double xCaret, yCaret;
extern double iVscrollPos;
char databuf[10000];
char textbuf[500][200];
int dlen = 0, wflag = 0, dcursor, cursorX = 0;
int DataLineWidth =124;
space* MainSpace, * DataSpace;
char* CopySpace, * SearchSpace, * SideSpace;
double textPosY;
int Line = 0;
extern int PatternFlag;
/* 鼠标和空间状态 */
typedef struct {
	double mousex;
	double mousey;
 ...
 *   1 - 被编辑了
 */
int magictextbox(int id, double x, double y, double w, double h)
{
	char* frameColor = gs_textbox_color.frame;
	char* labelColor = gs_textbox_color.label;
	int textChanged = 0;
	int len = strlen(GB_array(MainSpace));
	double indent = GetFontAscent() / 2;
	textPosY = y + iVscrollPos + h / 2 - GetFontAscent() / 2 - 0.3;

	if (notInMenu(gs_UIState.mousex, gs_UIState.mousey) &&
		inBox(gs_UIState.mousex, gs_UIState.mousey, x, x + w, y + iVscrollPos, y + h + iVscrollPos))
	{
	...
	// Render the text box
	mySetPenColor(frameColor);
	drawRectangle(x, y + iVscrollPos, w, h, gs_textbox_color.fillflag);
	// show text
	mySetPenColor(labelColor);
	/*
	MovePen(1, 1);
	DrawTextString(GB_array(DataSpace));
	*/
	char* ArrayDataSpace = GB_array(DataSpace);
	int ArrayIndexQuick = 0, ArrayLength = 0, ArrayIndexSlow = 0;
	Line = 0;
	char* midbuf;
	while (ArrayDataSpace[ArrayIndexQuick] != '\0')
	{
		if (ArrayDataSpace[ArrayIndexQuick] == '0')
		{
			midbuf = (char*)malloc(sizeof(char) * 200);
			memcpy(midbuf, GB_array(MainSpace) + ArrayIndexSlow, ArrayLength);
			memcpy(textbuf[Line], midbuf, ArrayLength);
			free(midbuf);
			textbuf[Line][ArrayLength] = '\0';
			ArrayIndexQuick += 1;
			ArrayIndexSlow += ArrayLength;
			ArrayLength = 0;
			Line++;
			continue;
		}

		if (ArrayLength == DataLineWidth)
		{
			midbuf = (char*)malloc(sizeof(char) * 200);
			memcpy(midbuf, GB_array(MainSpace) + ArrayIndexSlow, ArrayLength);
			memcpy(textbuf[Line], midbuf, ArrayLength);
		...
		if (ArrayLength > DataLineWidth)
		{
			...
			memcpy(textbuf[Line], midbuf, ArrayLength);
			free(midbuf);
			textbuf[Line][ArrayLength - 2] = '\0';
			ArrayIndexQuick -= 1;
			ArrayIndexSlow += ArrayLength - 2;
			ArrayLength = 0;
			Line++;
			continue;
		}
		ArrayLength += ArrayDataSpace[ArrayIndexQuick] - '0';
		ArrayIndexQuick++;
	}
	midbuf = (char*)malloc(sizeof(char) * 200);
	memcpy(midbuf, GB_array(MainSpace) + ArrayIndexSlow, ArrayLength);
	...
	for (i = 0; i <= Line; i++)
	{
		MovePen(x + indent, textPosY - RowPitch * i); 
	...
	if (gs_UIState.kbdItem == id)
	{
		...
		case VK_LEFT:
			if (DataSpace->word[DataSpace->ahead - 1] == '2')
			{
				GB_backward(MainSpace);
				GB_backward(MainSpace);
				GB_backward(DataSpace);
				cursorX -= 2 * WordWidth;
				xCaret -= 2 * FontWidth1;
			}
		...
		case VK_RIGHT:
			if (DataSpace->word[DataSpace->ahead - 1] == '2')
			{
				GB_forward(MainSpace);
				GB_forward(MainSpace);
				GB_forward(DataSpace);
				cursorX += 2 * WordWidth;
				xCaret += 2 * FontWidth1;
			}
			else if (DataSpace->word[DataSpace->ahead - 1] == '1')
		...
		{
			GB_insert(DataSpace, '1');
			xCaret += FontWidth1;
			dcursor++;
		}

		if (gs_UIState.charInput < 0 && len + 1)
		{
			if (wflag == 0)
			{
				GB_insert(DataSpace, '2');
				xCaret += 2 * FontWidth1;
				wflag = 1;
				dcursor++;
			}
		...
		if (gs_UIState.charInput >= 32 && gs_UIState.charInput < 127 || gs_UIState.charInput < 0) {
			GB_insert(MainSpace, gs_UIState.charInput);
